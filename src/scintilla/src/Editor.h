...
class WorkNeeded {
 public:
  enum workItems { workNone = 0, workStyle = 1, workUpdateUI = 2 };
  ...
  void Need(workItems items_, Sci::Position pos) noexcept {
    if ((items_ & workStyle) && (upTo < pos)) upTo = pos;
...
class SelectionText {
 ...
 public:
  ...
  void Copy(const std::string& s_, int codePage_, int characterSet_, bool rectangular_,
            bool lineCopy_) {
    s = s_;
    codePage = codePage_;
...
class Editor : public EditModel, public DocWatcher {
 protected:  // ScintillaBase subclass needs access to much of Editor
  ...
  PointDocument DocumentPointFromView(
      Point ptView) const;                   // Convert a point from view space to document
  ...
  SelectionPosition SPositionFromLocation(Point pt, bool canReturnInvalid = false,
                                          bool charPosition = false, bool virtualSpace = true);
  Sci::Position PositionFromLocation(Point pt, bool canReturnInvalid = false,
  ...
  void MovePositionTo(SelectionPosition newPos, Selection::selTypes selt = Selection::noSel,
                      bool ensureVisible = true);
  void MovePositionTo(Sci::Position newPos, Selection::selTypes selt = Selection::noSel,
                      bool ensureVisible = true);
  ...
  struct XYScrollPosition {
    int xOffset;
    Sci::Line topLine;
    XYScrollPosition(int xOffset_, Sci::Line topLine_) noexcept
        : xOffset(xOffset_), topLine(topLine_) {
    }
  ...
  void DropAt(SelectionPosition position, const char* value, size_t lengthValue, bool moving,
              bool rectangular);
