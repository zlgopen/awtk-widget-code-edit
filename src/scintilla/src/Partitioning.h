...
class SplitVectorWithRangeAdd : public SplitVector<T> {
 public:
  ...
  void RangeAddDelta(ptrdiff_t start, ptrdiff_t end, T delta) noexcept {
    ...
    if (range1Length > part1Left) range1Length = part1Left;
...
class Partitioning {
 private:
  ...
  void Allocate(ptrdiff_t growSize) {
    ...
    body->Insert(1,
                 0);  // This is the end of the first partition and will be the start of the second
  }
 ...
 public:
  ...
  T PositionFromPartition(T partition) const noexcept {
    ...
    if (partition > stepPartition) pos += stepLength;
    return pos;
  }
  ...
  T PartitionFromPosition(T pos) const noexcept {
    ...
    if (pos >= (PositionFromPartition(Partitions()))) return Partitions() - 1;
    T lower = 0;
    T upper = Partitions();
    do {
      const T middle = (upper + lower + 1) / 2;  // Round high
      T posMiddle = body->ValueAt(middle);
      if (middle > stepPartition) posMiddle += stepLength;
      if (pos < posMiddle) {
        upper = middle - 1;
      } else {
        lower = middle;
