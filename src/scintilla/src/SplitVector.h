...
class SplitVector {
 protected:
  ...
  void RoomFor(ptrdiff_t insertionLength) {
    if (gapLength <= insertionLength) {
      while (growSize < static_cast<ptrdiff_t>(body.size() / 6)) growSize *= 2;
 ...
 public:
  ...
  void ReAllocate(ptrdiff_t newSize) {
    if (newSize < 0) throw std::runtime_error("SplitVector::ReAllocate: negative size.");
  ...
  void InsertFromArray(ptrdiff_t positionToInsert, const T s[], ptrdiff_t positionFrom,
