...
bool CanDeferToLastStep(const DocModification& mh) noexcept {
  if (mh.modificationType & (SC_MOD_BEFOREINSERT | SC_MOD_BEFOREDELETE)) return true;  // CAN skip
  if (!(mh.modificationType & (SC_PERFORMED_UNDO | SC_PERFORMED_REDO))) return false;  // MUST do
...
SelectionPosition Editor::SPositionFromLocation(Point pt, bool canReturnInvalid, bool charPosition,
                                                bool virtualSpace) {
  RefreshStyleData();
  AutoSurface surface(this);
...
void Editor::SetRectangularRange() {
  if (sel.IsRectangular()) {
    ...
    const Sci::Line lineAnchorRect = pdoc->SciLineFromPosition(sel.Rectangular().anchor.Position());
    const Sci::Line lineCaret = pdoc->SciLineFromPosition(sel.Rectangular().caret.Position());
    ...
    for (std::vector<Range>::const_iterator it = searchRanges.begin(); it != searchRanges.end();
      ...
      for (;;) {
        ...
        const Sci::Position pos =
            pdoc->FindText(searchStart, searchEnd, selectedText.c_str(), searchFlags, &lengthFound);
        if (pos >= 0) {
...
SelectionPosition Editor::MovePositionOutsideChar(SelectionPosition pos, Sci::Position moveDir,
                                                  bool checkLineEnd) const {
...
void Editor::MovePositionTo(SelectionPosition newPos, Selection::selTypes selt,
                            bool ensureVisible) {
  const SelectionPosition spCaret =
      ((sel.Count() == 1) && sel.Empty()) ? sel.Last() : SelectionPosition(INVALID_POSITION);
...
void Editor::MoveSelectedLines(int lineDelta) {
  ...
  if (selectionEnd > beginningOfEndLine || selectionStart == selectionEnd) {
    ...
    appendEol =
        (selectionEnd == pdoc->Length() && pdoc->SciLineFromPosition(selectionEnd) == endLine);
  }

  // if there's nowhere for the selection to move
  // (i.e. at the beginning going up or at the end going down),
  // stop it right there!
  if ((selectionStart == 0 && lineDelta < 0) || (selectionEnd == pdoc->Length() && lineDelta > 0) ||
  ...
  if (appendEol) SetSelection(pdoc->MovePositionOutsideChar(selectionStart - 1, -1), selectionEnd);
  ClearSelection();
    ...
    if (bSlop) {  // A margin is defined
      ...
      if (bStrict) {
        ...
        if (!(options & xysUseMargin)) {
          ...
          yMarginT = Sci::clamp(static_cast<Sci::Line>(policies.y.slop), static_cast<Sci::Line>(1),
                                halfScreen);
          if (bEven) {
            yMarginB = yMarginT;
          } else {
        ...
        if (bEven) {
          if (bJump) {
            yMoveT = Sci::clamp(static_cast<Sci::Line>(policies.y.slop * 3),
...
void Editor::EnsureCaretVisible(bool useMargin, bool vert, bool horiz) {
  SetXYScroll(XYScrollToMakeVisible(
      ...
      static_cast<XYScrollOptions>((useMargin ? xysUseMargin : 0) | (vert ? xysVertical : 0) |
                                   (horiz ? xysHorizontal : 0)),
      caretPolicies));
}
...
bool Editor::WrapOneLine(Surface* surface, Sci::Line lineToWrap) {
  ...
  return pcs->SetHeight(
      lineToWrap, linesWrapped + (vs.annotationVisible ? pdoc->AnnotationLines(lineToWrap) : 0));
}
  ...
  if (!Wrapping()) {
    if (wrapWidth != LineLayout::wrapWidthInfinite) {
      ...
      for (Sci::Line lineDoc = 0; lineDoc < pdoc->LinesTotal(); lineDoc++) {
        pcs->SetHeight(lineDoc, 1 + (vs.annotationVisible ? pdoc->AnnotationLines(lineDoc) : 0));
      }
      wrapOccurred = true;
    }
    wrapPending.Reset();
    ...
    if (ws == WrapScope::wsVisible) {
      ...
      const Sci::Line linesInAllowedTime =
          Sci::clamp(static_cast<Sci::Line>(secondsAllowed / durationWrapOneLine.Duration()),
    ...
    if (lineToWrap < lineToWrapEnd) {
      ...
      if (surface) {
        ...
        goodTopLine = pcs->DisplayFromDoc(lineDocTop) +
      ...
      if (surface && ll) {
        ...
        for (int subLine = 1; subLine < ll->lines; subLine++) {
          const Sci::Position lengthInserted = pdoc->InsertString(
              posLineStart + lengthInsertedTotal + ll->LineStart(subLine), eol, strlen(eol));
          targetRange.end.Add(lengthInserted);
...
void Editor::RefreshPixMaps(Surface* surfaceWindow) {
  ...
  if (view.bufferedDraw) {
    ...
    if (!view.pixmapLine->Initialised()) {
      view.pixmapLine->InitPixMap(static_cast<int>(rcClient.Width()), vs.lineHeight, surfaceWindow,
                                  wMain.GetID());
    }
    if (!marginView.pixmapSelMargin->Initialised()) {
      marginView.pixmapSelMargin->InitPixMap(
          vs.fixedColumnWidth, static_cast<int>(rcClient.Height()), surfaceWindow, wMain.GetID());
    }
  }
}
...
int Editor::TextWidth(int style, const char* text) {
  ...
  if (surface) {
    return static_cast<int>(
        surface->WidthText(vs.styles[style].font, text, static_cast<int>(strlen(text))));
  } else {
    return 1;
  }
}
...
void Editor::InsertPasteShape(const char* text, Sci::Position len, PasteShape shape) {
  ...
  if (shape == pasteRectangular) {
    ...
    if (shape == pasteLine) {
      const Sci::Position insertPos = pdoc->LineStart(pdoc->LineFromPosition(sel.MainCaret()));
...
void Editor::PasteRectangular(SelectionPosition pos, const char* ptr, Sci::Position len) {
  ...
  bool prevCr = false;
  while ((len > 0) && IsEOLChar(ptr[len - 1])) len--;
      ...
      if (!RangeContainsProtected(sel.Range(r).caret.Position(),
        ...
        if (sel.Range(r).Start().VirtualSpace()) {
          if (sel.Range(r).anchor < sel.Range(r).caret)
            sel.Range(r) = SelectionRange(RealizeVirtualSpace(sel.Range(r).anchor.Position(),
                                                              sel.Range(r).anchor.VirtualSpace()));
          else
            sel.Range(r) = SelectionRange(RealizeVirtualSpace(sel.Range(r).caret.Position(),
    ...
    for (size_t r = 0; r < sel.Count(); r++) {
      if (!RangeContainsProtected(sel.Range(r).caret.Position() - 1,
        ...
        if (sel.Range(r).caret.VirtualSpace()) {
          ...
          const Sci::Line lineCurrentPos = pdoc->SciLineFromPosition(sel.Range(r).caret.Position());
...
Sci::Position MovePositionForInsertion(Sci::Position position, Sci::Position startInsertion,
...
Sci::Position MovePositionForDeletion(Sci::Position position, Sci::Position startDeletion,
                                      Sci::Position length) noexcept {
...
void Editor::NotifyModified(Document*, DocModification mh, void*) {
  ...
  if (mh.modificationType & (SC_MOD_CHANGESTYLE | SC_MOD_CHANGEINDICATOR)) {
    ...
    if (mh.linesAdded != 0) {
      ...
      if (mh.position > pdoc->LineStart(lineOfPos)) lineOfPos++;  // Affecting subsequent lines
      if (mh.linesAdded > 0) {
    ...
    if (mh.modificationType & SC_MOD_CHANGEANNOTATION) {
      ...
      if (vs.annotationVisible) {
        if (pcs->SetHeight(lineDoc,
                           pcs->GetHeight(lineDoc) + static_cast<int>(mh.annotationLinesAdded))) {
          SetScrollBars();
        }
        Redraw();
      }
    ...
    if (mh.linesAdded != 0) {
      ...
      if (mh.position < posTopLine && !CanDeferToLastStep(mh)) {
        const Sci::Line newTop =
            Sci::clamp(topLine + mh.linesAdded, static_cast<Sci::Line>(0), MaxScrollPos());
        if (newTop != topLine) {
          SetTopLine(newTop);
          SetVerticalScrollPos();
        }
...
void Editor::PageMove(int direction, Selection::selTypes selt, bool stuttered) {
  ...
  const Sci::Line bottomStutterLine =
      pdoc->SciLineFromPosition(PositionFromLocation(Point::FromInts(
          lastXChosen - xOffset, direction * vs.lineHeight * static_cast<int>(LinesToScroll())))) -
      caretPolicies.y.slop - 1;

  if (stuttered && (direction < 0 && currentLine > topStutterLine)) {
    topLineNew = topLine;
    newPos = SPositionFromLocation(
        Point::FromInts(lastXChosen - xOffset, vs.lineHeight * caretPolicies.y.slop), false, false,
        UserVirtualSpace());
    ...
    newPos = SPositionFromLocation(
        Point::FromInts(lastXChosen - xOffset,
                        vs.lineHeight * static_cast<int>(LinesToScroll() - caretPolicies.y.slop)),
        false, false, UserVirtualSpace());

  } else {
    const Point pt = LocationFromPosition(sel.MainCaret());

    topLineNew = Sci::clamp(topLine + direction * LinesToScroll(), static_cast<Sci::Line>(0),
                            MaxScrollPos());
    newPos = SPositionFromLocation(
        Point::FromInts(lastXChosen - xOffset,
  ...
  for (size_t r = 0; r < sel.Count(); r++) {
    ...
    if (rangeBytes > 0) {
      ...
      if (sMapped != sText) {
        ...
        while (sMapped[firstDifference] == sText[firstDifference]) firstDifference++;
...
void Editor::LineTranspose() {
  ...
  if (line > 0) {
    ...
    startCurrent += pdoc->InsertString(startPrevious, lineCurrent.c_str(), lineCurrent.length());
...
void Editor::CursorUpOrDown(int direction, Selection::selTypes selt) {
  ...
  if (selt == Selection::selRectangle) {
    ...
    const SelectionPosition posNew =
        MovePositionSoVisible(PositionUpOrDown(caretToUse, direction, lastXChosen), direction);
    ...
    for (size_t r = 0; r < sel.Count(); r++) {
      ...
      sel.Range(r) = selt == Selection::selStream ? SelectionRange(posNew, sel.Range(r).anchor)
  ...
  for (size_t r = 0; r < sel.Count(); r++) {
    ...
    switch (iMessage) {
      ...
      case SCI_DELWORDRIGHTEND:
        ...
        break;
      case SCI_DELLINELEFT:
        rangeDelete = Range(pdoc->LineStart(pdoc->LineFromPosition(sel.Range(r).caret.Position())),
                            sel.Range(r).caret.Position());
        break;
      case SCI_DELLINERIGHT:
        rangeDelete = Range(sel.Range(r).caret.Position(),
...
void Editor::Indent(bool forwards) {
  UndoGroup ug(pdoc);
  for (size_t r = 0; r < sel.Count(); r++) {
    const Sci::Line lineOfAnchor = pdoc->SciLineFromPosition(sel.Range(r).anchor.Position());
    ...
    if (lineOfAnchor == lineCurrentPos) {
      if (forwards) {
        ...
        if (pdoc->GetColumn(caretPosition) <=
          ...
          if (pdoc->useTabs) {
            ...
            int numSpaces =
                (pdoc->tabInChars) - (pdoc->GetColumn(caretPosition) % (pdoc->tabInChars));
            if (numSpaces < 1) numSpaces = pdoc->tabInChars;
        ...
        if (pdoc->GetColumn(caretPosition) <= pdoc->GetLineIndentation(lineCurrentPos) &&
          ...
          Sci::Position newColumn =
              ((pdoc->GetColumn(caretPosition) - 1) / pdoc->tabInChars) * pdoc->tabInChars;
          if (newColumn < 0) newColumn = 0;
          Sci::Position newPos = caretPosition;
          while (pdoc->GetColumn(newPos) > newColumn) newPos--;
      ...
      if (lineOfAnchor < lineCurrentPos) {
        if (currentPosPosOnLine == 0)
          sel.Range(r) =
              SelectionRange(pdoc->LineStart(lineCurrentPos), pdoc->LineStart(lineOfAnchor));
        else
          sel.Range(r) =
              SelectionRange(pdoc->LineStart(lineCurrentPos + 1), pdoc->LineStart(lineOfAnchor));
      } else {
        if (anchorPosOnLine == 0)
          ...
          sel.Range(r) =
              SelectionRange(pdoc->LineStart(lineCurrentPos), pdoc->LineStart(lineOfAnchor + 1));
      }
    }
  }
  ContainerNeedsUpdate(SC_UPDATE_SELECTION);
...
Sci::Position Editor::FindText(
  ...
  if (!pdoc->HasCaseFolder()) pdoc->SetCaseFolder(CaseFolderForEncoding());
  try {
    const Sci::Position pos = pdoc->FindText(static_cast<Sci::Position>(ft->chrg.cpMin),
                                             static_cast<Sci::Position>(ft->chrg.cpMax),
                                             ft->lpstrText, static_cast<int>(wParam), &lengthFound);
    if (pos != -1) {
...
Sci::Position Editor::SearchText(
  ...
  try {
    if (iMessage == SCI_SEARCHNEXT) {
      pos =
          pdoc->FindText(searchAnchor, pdoc->Length(), txt, static_cast<int>(wParam), &lengthFound);
    } else {
...
Sci::Position Editor::SearchInTarget(const char* text, Sci::Position length) {
  ...
  try {
    const Sci::Position pos = pdoc->FindText(
        targetRange.start.Position(), targetRange.end.Position(), text, searchFlags, &lengthFound);
    if (pos != -1) {
...
void Editor::CopySelectionRange(SelectionText* ss, bool allowLineCopy) {
  if (sel.Empty()) {
    if (allowLineCopy) {
      ...
      ss->Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, false, true);
    }
  } else {
    std::string text;
    ...
    ss->Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, sel.IsRectangular(),
...
void Editor::DropAt(SelectionPosition position, const char* value, size_t lengthValue, bool moving,
                    bool rectangular) {
  ...
  if ((inDragDrop != ddDragging) || !(positionWasInSelection) ||
    ...
    if (rectangular) {
      ...
      const Sci::Position lengthInserted =
          pdoc->InsertString(position.Position(), convertedText.c_str(), convertedText.length());
      if (lengthInserted > 0) {
...
void Editor::ButtonMoveWithModifiers(Point pt, unsigned int, int modifiers) {
  ...
  if (HaveMouseCapture()) {
    ...
    if (autoScrollTimer.ticksToWait > 0) return;
...
Sci::Position Editor::PositionAfterMaxStyling(Sci::Position posMax, bool scrolling) const {
  ...
  const Sci::Line linesToStyle = Sci::clamp(
      static_cast<int>(secondsAllowed / pdoc->durationStyleOneLine.Duration()), 10, 0x10000);
  const Sci::Line stylingMaxLine =
      std::min(pdoc->SciLineFromPosition(pdoc->GetEndStyled()) + linesToStyle, pdoc->LinesTotal());
...
void Editor::SetAnnotationHeights(Sci::Line start, Sci::Line end) {
  if (vs.annotationVisible) {
    ...
    for (Sci::Line line = start; line < end && line < pdoc->LinesTotal(); line++) {
      ...
      if (pcs->SetHeight(line, pdoc->AnnotationLines(line) + linesWrapped)) changedHeight = true;
    }
    if (changedHeight) {
      Redraw();
    }
...
Sci::Line Editor::ContractedFoldNext(Sci::Line lineStart) const {
  for (Sci::Line line = lineStart; line < pdoc->LinesTotal();) {
    if (!pcs->GetExpanded(line) && (pdoc->GetLevel(line) & SC_FOLDLEVELHEADERFLAG)) return line;
  ...
  if (enforcePolicy) {
    ...
    if (visiblePolicy.policy & VISIBLE_SLOP) {
      if ((topLine > lineDisplay) || ((visiblePolicy.policy & VISIBLE_STRICT) &&
        ...
        SetTopLine(Sci::clamp(lineDisplay - visiblePolicy.slop, static_cast<Sci::Line>(0),
                              MaxScrollPos()));
        SetVerticalScrollPos();
        Redraw();
        ...
        SetTopLine(Sci::clamp(lineDisplay - LinesOnScreen() + 1 + visiblePolicy.slop,
                              static_cast<Sci::Line>(0), MaxScrollPos()));
        SetVerticalScrollPos();
        Redraw();
      }
    } else {
      if ((topLine > lineDisplay) || (lineDisplay > topLine + LinesOnScreen() - 1) ||
          (visiblePolicy.policy & VISIBLE_STRICT)) {
        SetTopLine(Sci::clamp(lineDisplay - LinesOnScreen() / 2 + 1, static_cast<Sci::Line>(0),
                              MaxScrollPos()));
        SetVerticalScrollPos();
        Redraw();
      }
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
  ...
  switch (iMessage) {
    ...
    case SCI_POSITIONRELATIVE:
      return Sci::clamp(pdoc->GetRelativePosition(static_cast<Sci::Position>(wParam), lParam),
                        static_cast<Sci::Position>(0), pdoc->Length());

    case SCI_POSITIONRELATIVECODEUNITS:
      return Sci::clamp(pdoc->GetRelativePositionUTF16(static_cast<Sci::Position>(wParam), lParam),
                        static_cast<Sci::Position>(0), pdoc->Length());
    ...
    case SCI_GETTEXTRANGE: {
      ...
      Sci::Position len =
          cpMax -
          tr->chrg.cpMin;  // No -1 as cpMin and cpMax are referring to inter character positions
    ...
    case SCI_MARKERSETFORE:
      if (wParam <= MARKER_MAX) vs.markers[wParam].fore = ColourDesired(static_cast<int>(lParam));
      InvalidateStyleData();
      RedrawSelMargin();
      break;
    ...
    case SCI_MARKERSETBACK:
      if (wParam <= MARKER_MAX) vs.markers[wParam].back = ColourDesired(static_cast<int>(lParam));
      InvalidateStyleData();
      RedrawSelMargin();
      break;
    ...
    case SCI_MARKERADDSET:
      if (lParam != 0) pdoc->AddMarkSet(static_cast<Sci::Line>(wParam), static_cast<int>(lParam));
      break;
